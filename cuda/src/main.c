...
static void print_usage(const char *prog) {
    fprintf(stderr,
        ...
        "  -t <threads>      Number of OpenMP threads (used for OBJ export) (default: automatic)\n"
...
int main(int argc, char **argv) {
    ...
    cuda_upload_perlin_permutation(perlin_state.p);
    cuda_upload_mc_tables();
    ...
    cuda_generate_heightmap(&d_heightmap, Nx, Nz, params.frequency, base_height, params.amplitude, &t_field);

    // Marching Cubes
    double t_mc = 0.0;
    TriMesh mesh = cuda_marching_cubes(d_heightmap, Nx, Ny, Nz, params.isovalue, &t_mc);

    cuda_free_heightmap(d_heightmap);
